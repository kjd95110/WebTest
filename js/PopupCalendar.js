...
if(dom) {
	...
	strCalendar += "td {font-size:11px; font-family:verdana; text-decoration:none; }";
	...
	strCalendar += "font { font-size: 11px; }";
	strCalendar += ".cnj_close {font-size:11px;color:#000000; background-color:#EFEFEF; border-width:1px; border-color:#808080; border-style:solid;cursor:pointer;font-weight:bold;height:16px;width:16px;text-align:center;vertical-align:bottom}";
	strCalendar += ".cnj_close2 {font-size:11px;color:#000000; background-color:#FFFFFF; border-width:1px; border-color:#808080; border-style:solid;cursor:pointer;font-weight:bold;height:16px;width:16px;text-align:center;vertical-align:bottom}";
	strCalendar += ".cnj_input {background-color:rgb(240,240,240);border-width:1px; height:16pt;cursor:pointer;}";
	strCalendar += ".cnj_input2 {width:16px;font-size:11px;color:#808080; background-color:#EFEFEF; border-width:1px; border-color:#808080; border-style:solid;cursor:pointer;height:16px;}";
	strCalendar += ".cnj_input3 {width:16px;font-size:11px;color:#000000; background-color:#FFFFFF; border-width:1px; border-color:#C00000; border-style:solid;cursor:pointer;height:16px;}";
	strCalendar += ".cnj_input4 {font-size:11px;color:#C00000; background-color:#FFFFFF; border-width:1px; border-color:#808080; border-style:solid;cursor:pointer;height:16px;}";
	strCalendar += ".cnj_td {border-width:1px;border-style:solid;border-color:#a0a0a0;}";
	strCalendar += "</style>";

    strCalendar += "<div id='calendar' style='z-index:+999;position:absolute;'>";
	strCalendar += "<table width='205px' class='cnj_td'>";
	strCalendar += "	<tr style='background-color:#EEEEEE;height:20px'>";
	strCalendar += "		<td>";
	strCalendar += "			<table width='195px' border='0px'>";
	strCalendar += "				<tr style='height:20px'>";
	...
	strCalendar += "		</td>";
	strCalendar += "	</tr>";
	strCalendar += "	<tr style='height:1px'>";
	strCalendar += "		<td style='padding:3px;background-color:#ffffff'><span id='content'></span></td>";
	strCalendar += "	</tr>";
			
	if(showToday==1) {
		strCalendar += "<tr style='background-color:#f0f0f0;height20px;'><td style='padding:5px' align=center><span id='lblToday'></span></td></tr>";
	}
...
function init() {
	if(!ns4) {
		...
		if(showToday==1) {
			...
			" style='" + styleAnchor + "' onclick='javascript:parent.monthSelected=" + monthNow + ";parent.yearSelected=" + yearNow + ";parent.constructCalendar();' onFocus='this.blur()'>" +
			"오늘 날짜 :  "+yearNow+"년 "+
		...
		sHTML1+="onMouseover=\"this.className='cnj_input4';window.status='년도 선택'\" onMouseout=\"this.className='cnj_input4';window.status=''\"></span>&nbsp;";

		sHTML1+="<span id='spanMonth'  class='cnj_input4' onclick='parent.popUpMonth()' title='월 선택' "
		sHTML1+="onMouseover=\"this.className='cnj_input4';window.status='월 선택'\" onMouseout=\"this.className='cnj_input4';window.status=''\"></span> ";
...
var styleAnchor = "text-decoration:none;color:black;cursor:pointer;width:100%;height:100%";
var styleLightBorder = "border-style:solid;border-width:1px;border-color:#a0a0a0;text-decoration:underline;font-weight:bold;cursor:pointer;width:100%;height:100%";
...
function constructMonth() {
	...
	if(!monthConstructed) { // 월 이전 월 링크
		...
		"<table width='50' style='font-family:굴림; font-size:11px; border-width:1px; border-style:solid; border-color:#a0a0a0;' bgcolor='#FFFFDD' "+ 
...
function popUpMonth() {
	...
	crossMonthObj.style.left = crossobj.style.left + 118;
...
function constructYear() {
	...
	if(!yearConstructed) { // 년도 이전 년도 링크
		...
		"<table width='55' style='font-family:굴림; font-size:11px; border-width:1px; border-style:solid; border-color:#a0a0a0;' bgcolor='#FFFFDD' "+ 
		" onmouseover='clearTimeout(parent.timeoutID2)' "+
...
function constructCalendar() {
	...
	if (intWeekCount == 6)
		...
		popHeight = 179;
	oPopup.show(cleft, ctop, 206, popHeight, document.body);
}
